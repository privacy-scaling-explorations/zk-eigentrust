use ecc::{maingate::RegionCtx, AssignedPoint, EccConfig, GeneralEccChip};
use halo2wrong::halo2::{
	arithmetic::{CurveAffine, FieldExt},
	plonk::Error,
};
use integer::{rns::Integer, AssignedInteger, IntegerChip, IntegerConfig, IntegerInstructions};
	...
	EcdsaChip<E, N, NUMBER_OF_LIMBS, BIT_LEN_LIMB>
{
	pub fn verify(
		...
		selector: &AssignedInteger<E::Scalar, N, NUMBER_OF_LIMBS, BIT_LEN_LIMB>,
	) -> Result<(), Error> {
		...
		let selected_q_x = scalar_chip.mul(ctx, &q_x_reduced_in_r, selector)?;
		let selected_sig_r = scalar_chip.mul(ctx, &sig.r, selector)?;
		scalar_chip.assert_strict_equal(ctx, &selected_q_x, &selected_sig_r)?;
...
mod tests {
	...
	use ecc::{
		integer::Range,
		maingate::{big_to_fe, fe_to_big, RegionCtx},
		EccConfig, GeneralEccChip,
	};
	use halo2wrong::{
		curves::group::{Curve, Group},
		halo2::{
		...
		fn synthesize(
			...
			layouter.assign_region(
				...
				|mut region| {
					...
					let selector = ecc_chip.new_unassigned_scalar(Some(E::Scalar::one()));
					...
					let assigned_selector = scalar_chip.assign_integer(ctx, selector, Range::Remainder)?;
					...
					ecdsa_chip.verify(ctx, &sig, &pk_assigned, &msg_hash, &assigned_selector)
				},
			)?;

...
pub struct Signature {
	...
	pub(crate) neighbours: [Option<Scalar>; MAX_NEIGHBORS],
	pub(crate) scores: [Option<Scalar>; MAX_NEIGHBORS],
}

impl Signature {
	pub fn new(
		sk: SecretKey, pk: PublicKey, neighbours: [Option<Scalar>; MAX_NEIGHBORS],
...
impl From<SignatureData> for Signature {
	fn from(sig: SignatureData) -> Self {
		...
		for (i, n) in sig.neighbours.iter().enumerate().take(MAX_NEIGHBORS) {
			neighbours[i] = Some(Scalar::from_repr(*n).unwrap());
		}

...
impl Debug for Opinion {
	fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		write!(
			...
			self.k, self.local_trust_score, self.global_trust_score
		)
	}
}
...
impl Opinion {
	pub fn generate(
		...
		let c_scaled = Bn256Scalar::from_u128((local_trust_score * SCALE) as u128);
		let t_scaled = Bn256Scalar::from_u128((global_trust_score * SCALE) as u128);
	...
	pub fn empty(keypair: &IdentityKeypair, pubkey_v: &PublicKey, k: Epoch) -> Self {
		Self::generate(keypair, pubkey_v, k, 0., 0.)
	}

	pub fn verify(&self, pubkey_v: &PublicKey) -> bool {
		if self.global_trust_score == 0. {
			return true;
		}
		verify_signature(
			&self.sig,
			&convert_pubkey(pubkey_v),
		)
	}
}
...
mod test {
	...
	use eigen_trust_circuit::halo2wrong::curves::secp256k1::{Fq as Secp256k1Scalar};
	use eigen_trust_circuit::halo2wrong::halo2::arithmetic::Field;
	use rand::thread_rng;
	...
	fn test_new_opinion() {
		...
		assert_eq!(opinon.local_trust_score, local_trust_score);
		assert_eq!(opinon.global_trust_score, global_trust_score);
	...
	fn test_generate_opinion() {
		...
		let opinion = Opinion::generate(&keypair_i, &pubkey_v, k, local_trust_score, global_trust_score);

...
impl EigenTrustClient {
	...
	pub async fn attest(&self) -> Result<(), ClientError> {
		...
		// 	calculate_message_hash::<NUM_NEIGHBOURS, 1>(user_publics.to_vec(),
		// vec![ops.to_vec()]);
...
#[cfg(test)]
mod test {
	use crate::{ClientConfig, EigenTrustClient};
